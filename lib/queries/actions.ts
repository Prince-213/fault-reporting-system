...
export async function delegateStaffToReport({
  ...
  try {
    ...
    try {
        const { data: report } = await supabase.from("fault_reports").select("*").eq("id", reportId).single();
        const { data: team } = await supabase.from("report_team").select("*").eq("id", delegatedTo).single();
